...
import {
  isWordOverdue, getWordMemoryState, MEMORY_STATES,
 ...
 * 3순위: 단기(간격 21일 미만), next_review 임박 순
 * 4순위: 중기(간격 21~60일), next_review 임박 순
 * 5순위: 장기(간격 60일 이상), next_review 임박 순
...
export function sortByForgettingPriority(words) {
  ...
  const shortTerm = rest
    .filter(w => {
      ...
      return stability >= 0 && stability < STABILITY_LEAF_DAYS;
    })
    .sort((a, b) => {
  ...
  const mediumTerm = rest
    .filter(w => {
      const stability = w.fsrs?.stability ?? 0;
      return stability >= STABILITY_LEAF_DAYS && stability < STABILITY_CARROT_DAYS && !shortTermIds.has(w.id);
    })
    .sort((a, b) => {
      const dateA = getNextReviewDate(a) ?? new Date(8640000000000000);
      const dateB = getNextReviewDate(b) ?? new Date(8640000000000000);
  ...
  const longTerm = rest
    .filter(w => {
      const stability = w.fsrs?.stability ?? 0;
      return stability >= STABILITY_CARROT_DAYS && !shortTermIds.has(w.id) && !mediumTermIds.has(w.id);
    })
    .sort((a, b) => {
      const dateA = getNextReviewDate(a) ?? new Date(8640000000000000);
      const dateB = getNextReviewDate(b) ?? new Date(8640000000000000);
