...
import { toLocalDateString, STABILITY_LEAF_DAYS, STABILITY_CARROT_DAYS } from '../utils/common';
  ...
  const memoryStats = useMemo(() => {
    ...
    for (const sheet of vocabularySheets) {
      for (const w of (sheet.words || [])) {
        ...
        // 경계는 common.jsx 단일 소스 — 여기 숫자를 다시 적으면 홈 통계만 다른 말을 한다
        if (stability < STABILITY_LEAF_DAYS) shortTerm++;
        else if (stability < STABILITY_CARROT_DAYS) mediumTerm++;
        else longTerm++;
