...
//
// **다크에서는 바깥 그림자를 쓰지 않는다** — 파운데이션 "깊이는 밝기로"(dark-mode 문서).
//   핑크 글로우는 `0 10px 24px`라 버튼 아래로 34px 까지 번지는데, 아래 카드까지의 간격은
//   14px 뿐이다(히어로 -16px 흘러나옴 + 본문 pt-30px). 라이트에서는 흰 배경이 그 번짐을
//   흡수하지만, 다크(#111111)에서는 간격이 통째로 분홍으로 물들어 버튼과 연속 학습 카드가
//   한 덩어리로 붙어 보인다. 다크에서는 안쪽 하이라이트만 남기고, 떠 있다는 느낌은
//   핑크 면 자체의 밝기가 대신 말한다.
//
...
export const HOME_STATE_VIEW = {
  ...
  [HOME_STATES.DONE]: {
    mood: 'fresh',
    // 다섯 상태 중 여기만 '학습'이라는 말을 쓰고 있었다. 나머지는 돌보기·물주기·씨앗 심기라
