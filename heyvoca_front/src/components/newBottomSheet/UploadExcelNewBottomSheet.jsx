...
export const UploadExcelNewBottomSheet = () => {
  ...
  const handleFileSelect = (e) => {
    ...
    if (!lower.endsWith('.xlsx') && !lower.endsWith('.xls')) {
      setFileError('.xlsx 또는 .xls 파일만 선택할 수 있어요.');
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const result = await uploadExcelApi(selectedFile, title.trim(), color);
  ...
  return (
    <div className="relative">
      <div className="flex items-center justify-center p-[20px] pb-[0px]">
        <h1 className="text-[18px] font-[700] text-layout-black dark:text-layout-white">EXCEL 파일 불러오기</h1>
      </div>
      ...
      <div className="flex flex-col gap-[24px] max-h-[calc(90vh-47px)] p-[20px] pb-[105px] overflow-y-auto">
        ...
        <div className="flex flex-col gap-[8px]">
          ...
          <p className="text-[12px] text-layout-gray-400">
            Excel 헤더: W(단어), M(뜻), EE(예문), EK(예문 뜻)
          </p>
