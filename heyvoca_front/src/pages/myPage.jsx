...
const MyPage = () => {
  const { fetchUserProfile, fetchUserMainPage, fetchAchievementCriteria } = useUser();

  // 당겨서 새로고침 — 프로필·업적 기준·업적 진행(goals)을 함께 최신화한다.
  // 창고(farmItems)·황금 온실(goldenCnt)·초대 수(inviteCnt)는 myPage/Main.jsx 안에서만
  // 상태로 들고 있어 여기서 직접 갱신할 수 없다 — Main이 자체적으로 재조회 함수를
  // 노출하면(registerRefresh) 함께 기다린다.
  const mainRefreshRef = useRef(null);
  const registerMainRefresh = useCallback((fn) => { mainRefreshRef.current = fn; }, []);
  const handlePullToRefresh = useCallback(async () => {
    await Promise.all([
      fetchUserProfile(),
      fetchUserMainPage(),
      fetchAchievementCriteria(),
      mainRefreshRef.current ? mainRefreshRef.current() : Promise.resolve(),
    ]);
  }, [fetchUserProfile, fetchUserMainPage, fetchAchievementCriteria]);
  ...
  return (
    ...
    <PullToRefresh
      ...
      // fixedHeader의 렌더된 높이(상태바+55px)를 PullToRefresh가 자동으로 측정해 콘텐츠·
      // 인디케이터를 그만큼 밀어낸다 — 아래에 있던 수동 spacer div(paddingTop)는 그래서
      // 걷어냈다(PullToRefresh.jsx "헤더 두 종류" · "인디케이터 위치" 참고).
      fixedHeader={(
        /*
          헤더 고정 — 스크롤은 본문(콘텐츠 래퍼)만 하고 헤더는 화면 위에 그대로 머문다.
          PullToRefresh의 fixedHeader 슬롯은 당김 중 콘텐츠에 걸리는 transform 밖에 그려지므로
          (PullToRefresh.jsx 상단 주석 참고) 당겨도 헤더가 같이 밀려 내려오지 않는다.
    ...
    >
      <Main registerRefresh={registerMainRefresh} />
    </PullToRefresh>
  );
};
