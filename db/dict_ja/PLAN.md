...
- 앱 1.1.1 릴리스(2026-09-25): iOS 빌드 19 제출 → WAITING_FOR_REVIEW, Android versionCode 23 → IN_REVIEW(앱 커밋 eb48d5a·12fc99e). 빌드 함정 2건: Xcode 27에서 fmt 11.0.2 consteval 실패 → Podfile에서 fmt 타깃만 C++17; Play가 targetSdk 35 AAB를 403 거부(2026-08-31 정책) → compile/target/buildTools 36. Play는 06:26 자동 게시(안내값 android=1.1.1 자동 반영, verify-deploy 통과). iOS 빌드 19는 20:49 REJECTED(2.1.0 App Completeness, 런치 크래시) — 원인은 Xcode 27(iOS 27 SDK) 빌드의 UIScene 미채택(TN3187). SceneDelegate 채택(앱 커밋 ccfb637) → iOS 27 시뮬레이터 런치 확인 → 빌드 20 업로드·재제출(21:19) → 2026-09-26 승인·출시 완료(READY_FOR_DISTRIBUTION). 양쪽 스토어 1.1.1 게시 완료. 거절된 제출은 `reviewSubmissionItems` PATCH resolved=true → `reviewSubmissions` PATCH submitted=true 로 재제출(asc.mjs submit이 처리).
